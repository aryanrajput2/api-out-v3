...
// Handle booking detail page from dashboard
document.addEventListener('DOMContentLoaded', function() {
  // Check if we're on the booking-detail page with an ID parameter
  const urlParams = new URLSearchParams(window.location.search);
  const bookingId = urlParams.get('id');
  
  if (bookingId && window.location.pathname === '/ui/booking-detail') {
    console.log('[BOOKING_DETAIL] Loading booking detail for ID:', bookingId);
    
    // Hide all pages first - be very explicit
    ...
    if (bookingDetailPage) {
      ...
      console.log('[BOOKING_DETAIL] Showing booking detail page');
    }
    
    // Fetch and display the booking detail
    loadBookingDetailFromDashboard(bookingId);
  }
});
...
function displayResponseTimes() {
  ...
  if (hasAnyTime) {
    ...
    if (headerBoxes) {
      ...
      console.log('Response time cards inserted:', html.length, 'chars');
    }
    ...
    if (section) {
      ...
      console.log('Response times section shown');
    }
  } else {
    const section = document.getElementById('response-times-section');
    if (section) {
      section.style.display = 'none';
      console.log('Response times section hidden (no times to display)');
    }
  }
}
...
function switchToResultsPage(lastSearchBody, durationMs, data) {
  ...
  if (summary && lastSearchBody) {
    ...
    // Simple and beautiful results summary
    const summaryText = document.getElementById("results-summary-text");
    if (summaryText) {
      summaryText.innerHTML = `You hit <strong>${searchedIds}</strong> hotel${searchedIds !== 1 ? 's' : ''}, we got <strong>${returnedHotels}</strong> hotel${returnedHotels !== 1 ? 's' : ''} in the API response`;
    }
  }
}
...
async function fetchHotelDetails(hotelId, optionId) {
  ...
  try {
    ...
    const dynamicDurationMs = Math.round(performance.now() - dynamicStartTime);
    
    console.log('API_DYNAMIC_DETAIL_REQUEST', { duration: dynamicDurationMs, status: dynamicRes.status, ok: dynamicData.ok });
    
    // Track response time (dynamic only, not including static)
    journeyResponseTimes.dynamicDetail = dynamicDurationMs;
    displayResponseTimes();
    ...
    timerUI.innerHTML = formatDuration(dynamicDurationMs);
    timerUI.classList.remove("hidden");
...
function renderStaticDetailsOnly(staticData, durationMs) {
  ...
  if (validImages && validImages.length > 0) {
    ...
    setTimeout(() => {
      ...
      // Main gallery click handler
      if (mainGallery) {
        mainGallery.onclick = (e) => {
          e.stopPropagation();
          const idx = parseInt(mainGallery.getAttribute('data-idx'), 10);
          if (validImages[idx]) {
            openImageZoom(validImages[idx]);
          }
        };
        
        // Hover effect for main gallery
        mainGallery.onmouseover = () => {
          const overlay = mainGallery.querySelector('.zoom-overlay');
          if (overlay) overlay.style.opacity = '1';
        };
        mainGallery.onmouseout = () => {
          const overlay = mainGallery.querySelector('.zoom-overlay');
      ...
      thumbGalleries.forEach((thumb) => {
        thumb.onclick = (e) => {
          ...
          if (mainGallery) {
            const mainImg = mainGallery.querySelector('img');
            if (mainImg && validImages[idx]) {
              mainImg.src = validImages[idx];
              mainGallery.setAttribute('data-idx', idx);
            }
            
            // Update thumbnail borders
            thumbGalleries.forEach((t, i) => {
      ...
      console.log('Gallery click handlers attached successfully');
    }, 100);
  }
...
function renderHotelDetails(data) {
  ...
  // Remove loading indicator but preserve static details
  ...
  // Create a container for room options (don't replace existing static content)
  const roomOptionsContainer = document.createElement("div");
  ...
  hotel.options.forEach((option, idx) => {
    ...
    card.innerHTML = `
      <div class="room-details-section">
        <!-- Room Images Section -->
        ${option.roomInfo && option.roomInfo.length > 0 && option.roomInfo[0].images && option.roomInfo[0].images.length > 0 ? `
          <div style="margin-bottom: 16px; padding-bottom: 16px; border-bottom: 2px solid #f1f5f9;">
            <h4 style="margin: 0 0 12px 0; font-size: 0.95rem; font-weight: 600; color: var(--text-main); display: flex; align-items: center; gap: 8px;">
              <i class="ph ph-images" style="font-size: 1.1rem; color: var(--primary);"></i> Room Images
            </h4>
            <div style="display: grid; grid-template-columns: repeat(auto-fill, minmax(100px, 1fr)); gap: 10px;">
              ${option.roomInfo[0].images.slice(0, 6).map((img, imgIdx) => {
                const imgUrl = typeof img === 'string' ? img : (img.links?.Standard?.href || img.links?.XL?.href || img.links?.XXL?.href || img.url || img.imageUrl || '');
                return imgUrl ? `
                  <div style="position: relative; width: 100%; aspect-ratio: 1; border-radius: 10px; overflow: hidden; cursor: pointer; border: 2px solid #e2e8f0; transition: all 0.3s ease; box-shadow: 0 2px 8px rgba(0,0,0,0.1);" 
                    onmouseover="this.style.transform='scale(1.05)'; this.style.borderColor='var(--primary)'; this.style.boxShadow='0 4px 16px rgba(59, 130, 246, 0.2)'" 
                    onmouseout="this.style.transform='scale(1)'; this.style.borderColor='#e2e8f0'; this.style.boxShadow='0 2px 8px rgba(0,0,0,0.1)'"
                    onclick="event.stopPropagation(); openImageZoom('${imgUrl}')">
                    <img src="${imgUrl}" alt="Room image ${imgIdx + 1}" style="width: 100%; height: 100%; object-fit: cover;">
                    <div style="position: absolute; inset: 0; background: rgba(0,0,0,0); transition: background 0.3s ease; display: flex; align-items: center; justify-content: center;" onmouseover="this.style.background='rgba(0,0,0,0.3)'" onmouseout="this.style.background='rgba(0,0,0,0)'">
                      <i class="ph ph-magnifying-glass-plus" style="color: white; font-size: 1.5rem; filter: drop-shadow(0 2px 4px rgba(0,0,0,0.3));"></i>
                    </div>
                  </div>
                ` : '';
              }).join('')}
            </div>
            ${option.roomInfo[0].images.length > 6 ? `<p style="margin: 8px 0 0 0; font-size: 0.8rem; color: #64748b;">... and ${option.roomInfo[0].images.length - 6} more images</p>` : ''}
          </div>
        ` : ''}
        ...
        ${option.inclusions && option.inclusions.length > 0 ? '<div style="margin-top: 12px; background: linear-gradient(135deg, #e0f2fe 0%, #cffafe 100%); border: 1px solid rgba(34, 197, 94, 0.2); border-radius: 12px; padding: 14px;"><div style="display: flex; align-items: center; gap: 8px; margin-bottom: 10px;"><i class="ph ph-check-circle" style="font-size: 1.1rem; color: #059669;"></i><span style="font-size: 0.85rem; font-weight: 700; color: #059669; text-transform: uppercase; letter-spacing: 0.5px;">Inclusions & Amenities</span></div><div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(150px, 1fr)); gap: 8px;">' + option.inclusions.map(inc => '<div style="background: white; border-radius: 8px; padding: 10px; border: 1px solid rgba(34, 197, 94, 0.15);"><span style="font-size: 0.8rem; font-weight: 500; color: #065f46;">' + inc.replace(/^[A-Za-z]+\s+(?:instructions|special instructions)?:?\s*/i, '').trim().split('\n')[0] + '</span></div>').join('') + '</div></div>' : ''}
        
        <!-- GST Information Section -->
        <div style="margin-top: 12px; background: linear-gradient(135deg, #fef3c7 0%, #fde68a 100%); border: 1px solid rgba(217, 119, 6, 0.2); border-radius: 12px; padding: 14px; box-shadow: 0 4px 12px rgba(217, 119, 6, 0.1);">
          <div style="display: flex; align-items: center; gap: 8px; margin-bottom: 10px;">
            <i class="ph ph-receipt" style="font-size: 1.1rem; color: #b45309;"></i>
            <span style="font-size: 0.85rem; font-weight: 700; color: #b45309; text-transform: uppercase; letter-spacing: 0.5px;">GST Information</span>
          </div>
          <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 12px;">
            <div style="background: white; border-radius: 8px; padding: 10px; border: 1px solid rgba(217, 119, 6, 0.1);">
              <div style="font-size: 0.7rem; font-weight: 700; color: #b45309; text-transform: uppercase; letter-spacing: 0.5px; margin-bottom: 4px;">GST Type</div>
              <div style="font-size: 0.95rem; font-weight: 700; color: #92400e;">${gstType}</div>
            </div>
            <div style="background: white; border-radius: 8px; padding: 10px; border: 1px solid rgba(217, 119, 6, 0.1);">
              <div style="font-size: 0.7rem; font-weight: 700; color: #b45309; text-transform: uppercase; letter-spacing: 0.5px; margin-bottom: 4px;">GST Claimable Amount</div>
              <div style="font-size: 0.95rem; font-weight: 700; color: #92400e;">${currency} ${(option.pricing?.gstClaimableAmount ?? 0).toFixed(2)}</div>
            </div>
          </div>
        </div>
    ...
    roomOptionsContainer.appendChild(card);
  });
  
  // Append the room options container to the results container (preserving static details)
  resultsContainer.appendChild(roomOptionsContainer);
}
        ...
        <div style="background: rgba(248, 250, 252, 0.8); border: 1px solid #e2e8f0; border-radius: var(--radius-md); padding: 16px; margin-top: 20px;">
           ...
           <div class="hotel-tags" style="margin-top: 16px;">
             ...
             <span class="data-pill pill-neutral"><i class="ph ph-tag"></i> GST: ${option.compliance?.gstType || 'NA'}</span>
               ${panReqHtml}
               ${passReqHtml}
               ${data.onholdAllowed ? '<span class="data-pill pill-success"><i class="ph ph-clock-countdown"></i> Hold Allowed</span>' : '<span class="data-pill pill-danger" style="opacity:0.6;"><i class="ph ph-x-circle"></i> Hold Not Allowed</span>'}
           </div>
          ...
          <div style="padding:18px; background:white; display:flex; flex-direction:column; gap:14px;">
            <div style="display:flex; gap:10px;">
              ...
              <div style="flex:1;">
                ...
                <input id="delivery-phone" type="tel" value="1234567890" style="width:100%; padding:10px 14px; border:1.5px solid #e5e7eb; border-radius:10px; font-size:0.9rem; background:#f9fafb; transition:border-color 0.2s;" onfocus="this.style.borderColor='#4f46e5';this.style.background='white'" onblur="this.style.borderColor='#e5e7eb';this.style.background='#f9fafb'" />
              </div>
            </div>
            <div>
              ...
              <input id="delivery-email" type="email" value="v3testingtj@gmail.com" style="width:100%; padding:10px 14px; border:1.5px solid #e5e7eb; border-radius:10px; font-size:0.9rem; background:#f9fafb; transition:border-color 0.2s;" onfocus="this.style.borderColor='#059669';this.style.background='white'" onblur="this.style.borderColor='#e5e7eb';this.style.background='#f9fafb'" />
            </div>
          </div>
        </div>
...
function generateRandomName() {
  const firstNames = ['Aryan', 'Priya', 'Rohan', 'Ananya', 'Vikram', 'Neha', 'Aditya', 'Pooja', 'Rahul', 'Divya', 'Karan', 'Shreya', 'Nikhil', 'Isha', 'Sanjay', 'Anjali', 'Amit', 'Riya', 'Varun', 'Zara', 'Akshay', 'Nisha', 'Arjun', 'Kavya', 'Harsh', 'Simran', 'Rishabh', 'Diya', 'Siddharth', 'Avni'];
  const lastNames = ['Singh', 'Sharma', 'Patel', 'Kumar', 'Verma', 'Gupta', 'Reddy', 'Nair', 'Iyer', 'Desai', 'Joshi', 'Rao', 'Bhat', 'Menon', 'Chopra', 'Malhotra', 'Saxena', 'Trivedi', 'Pandey', 'Mishra', 'Sinha', 'Yadav', 'Agarwal', 'Bose', 'Dutta', 'Ghosh', 'Mukherjee', 'Banerjee', 'Roy', 'Das'];
...
function buildTravellerFormRows(searchBody) {
  ...
  const renderInput = (id, placeholder, label, defaultValue = '') =>
    `<div><label style="${labelStyle}">${label}</label><input id="${id}" type="text" placeholder="${placeholder}" value="${defaultValue}" style="${inputStyle}" /></div>`;
      ...
      rows += `
        <div class="traveller-row" data-room="${rIdx}" data-pax="ADULT"
          ...
          <div style="display:grid; grid-template-columns:100px 1fr 1fr; gap:10px;">
            ...
            ${renderInput(`t-fn-${tNum}`, 'First Name', 'First Name *', randomName.firstName)}
            ${renderInput(`t-ln-${tNum}`, 'Last Name', 'Last Name *', randomName.lastName)}
          </div>
...
async function submitBooking(bookingType, bookingId, correlationId, amount) {
  ...
  try {
    ...
    const containerClass = bookingType === 'HOLD' ? 'booking-success-container hold' : 'booking-success-container';
    ...
    bookingEl.innerHTML = `
      <div class="${containerClass}">
        ...
        <div style="display: flex; align-items: flex-start; gap: 16px; margin-bottom: 20px;">
          <div class="booking-icon" style="background: linear-gradient(135deg, ${successColor} 0%, ${successColor}dd 100%); width: 56px; height: 56px; border-radius: 12px; display: flex; align-items: center; justify-content: center; flex-shrink: 0; box-shadow: 0 4px 12px ${successColor}40;">
            <i class="ph ${successIcon}" style="color: white; font-size: 28px;"></i>
          </div>
          <div style="flex: 1;">
            <div style="color: ${successColor}; font-weight: 800; font-size: 1.3rem; line-height: 1.3; margin-bottom: 6px;">${successTitle}</div>
            <div style="color: ${successColor}99; font-size: 0.95rem; font-weight: 500;">${successMsg}</div>
          </div>
        </div>
        
        <!-- Booking Details Card -->
        <div style="background: white; border-radius: 12px; padding: 16px; margin-bottom: 16px; border: 1px solid ${successColor}15; backdrop-filter: blur(10px);">
          <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 16px;">
            <div>
              <div style="font-size: 0.75rem; font-weight: 700; color: #999; text-transform: uppercase; letter-spacing: 0.5px; margin-bottom: 6px;">Booking ID</div>
              <div style="font-size: 1.1rem; font-weight: 700; color: ${successColor}; font-family: 'Monaco', 'Courier New', monospace; word-break: break-all;">${data.bookingId || bookingId}</div>
            </div>
            <div>
              <div style="font-size: 0.75rem; font-weight: 700; color: #999; text-transform: uppercase; letter-spacing: 0.5px; margin-bottom: 6px;">Status</div>
              <div style="display: inline-block; background: ${successColor}15; color: ${successColor}; padding: 6px 12px; border-radius: 8px; font-weight: 700; font-size: 0.9rem; border: 1px solid ${successColor}30;">${data.status || 'Confirmed'}</div>
            </div>
          </div>
        </div>
        
        <!-- Additional Info -->
        <div style="background: ${successColor}08; border-radius: 10px; padding: 12px 14px; margin-bottom: 16px; border-left: 4px solid ${successColor}; font-size: 0.9rem; color: ${successColor}99;">
          <i class="ph ph-info" style="margin-right: 8px;"></i>
          <span style="font-weight: 500;">Your booking has been successfully created and is now active.</span>
        </div>
        
        <!-- Action Button -->
        <button onclick="viewBookingDetail('${data.bookingId || bookingId}')" style="width: 100%; padding: 14px 20px; background: linear-gradient(135deg, ${successColor} 0%, ${successColor}dd 100%); color: white; border: none; border-radius: 10px; font-weight: 700; cursor: pointer; display: flex; align-items: center; justify-content: center; gap: 10px; transition: all 0.3s ease; font-size: 0.95rem; box-shadow: 0 4px 15px ${successColor}40; text-transform: uppercase; letter-spacing: 0.5px;" onmouseover="this.style.transform='translateY(-2px)'; this.style.boxShadow='0 6px 20px ${successColor}60';" onmouseout="this.style.transform='translateY(0)'; this.style.boxShadow='0 4px 15px ${successColor}40';">
          <i class="ph ph-arrow-right"></i> View Full Booking Details
        </button>
      </div>
      
      <!-- Response JSON Display -->
      <div style="margin-top: 16px;">
        <div style="font-size: 0.8rem; font-weight: 700; color: #666; text-transform: uppercase; letter-spacing: 0.5px; margin-bottom: 8px;">API Response</div>
        <pre style="font-size: 0.75rem; background: linear-gradient(135deg, #f8faff 0%, #f0f4ff 100%); padding: 14px; border-radius: 10px; overflow-x: auto; border: 1px solid #e0e7ff; color: #1e293b; font-family: 'Monaco', 'Courier New', monospace; line-height: 1.5; box-shadow: inset 0 2px 4px rgba(0,0,0,0.02);">${JSON.stringify(data, null, 2)}</pre>
      </div>`;
  } catch (err) {
    if (!bookingEl) return;
    bookingEl.innerHTML = `
...
// Function to load booking detail from dashboard (with ID parameter)
async function loadBookingDetailFromDashboard(bookingId) {
  // Hide ALL pages first - be very explicit
  const searchPage = document.getElementById("search-page");
  const resultsPage = document.getElementById("results-page");
  const detailPage = document.getElementById("detail-page");
  ...
  try {
    ...
    console.log('DASHBOARD_BOOKING_DETAIL_REQUEST', { duration: responseMs, status: res.status, ok: data.ok });
    
    // Track booking detail response time
    ...
    renderBookingDetailFromDashboard(data);
...
// Function to render booking detail from dashboard
function renderBookingDetailFromDashboard(data) {
  const container = document.getElementById("booking-detail-content");
  if (!container) return;
  
  // Extract booking details from the API response
  const order = data.order || {};
  const itemInfos = data.itemInfos || {};
  const hotelInfo = itemInfos.HOTEL?.hInfo || {};
  const operations = hotelInfo.ops || [];
  const firstOp = operations[0] || {};
  const rooms = firstOp.ris || [];
  const firstRoom = rooms[0] || {};
  
  const bookingId = order.bookingId || 'N/A';
  const amount = order.amount || 0;
  const markup = order.markup || 0;
  const status = order.status || 'PENDING';
  const createdOn = order.createdOn || new Date().toISOString();
  const hotelName = hotelInfo.name || 'Hotel';
  const hotelRating = hotelInfo.rt || 0;
  const address = hotelInfo.ad || {};
  const checkInTime = hotelInfo.checkInTime || {};
  const checkOutTime = hotelInfo.checkOutTime || {};
  const roomName = firstRoom.rc || 'Room';
  const roomType = firstRoom.rt || 'Standard';
  const mealPlan = firstRoom.mb || 'Room Only';
  const travellers = firstRoom.ti || [];
  const roomPrice = firstRoom.tp || 0;
  const checkOutDate = firstRoom.ddt || 'N/A';
  ...
  const statusColor = status === 'ON_HOLD' ? '#f59e0b' : status === 'CONFIRMED' ? '#10b981' : '#6b7280';
  const statusIcon = status === 'ON_HOLD' ? 'ph-clock-countdown' : status === 'CONFIRMED' ? 'ph-check-circle' : 'ph-info';
  
  const bookingDetailsHtml = `
    <div style="display: flex; flex-direction: column; gap: 24px;">
      
      <!-- Header Banner -->
      <div style="background: linear-gradient(135deg, ${statusColor}15 0%, ${statusColor}08 100%); border: 2px solid ${statusColor}40; border-radius: 16px; padding: 24px; display: flex; align-items: center; gap: 16px; box-shadow: 0 10px 30px ${statusColor}20;">
        <div style="width: 60px; height: 60px; background: linear-gradient(135deg, ${statusColor} 0%, ${statusColor}dd 100%); border-radius: 14px; display: flex; align-items: center; justify-content: center; color: white; font-size: 1.8rem; flex-shrink: 0;">
          <i class="ph ${statusIcon}"></i>
        </div>
        <div style="flex: 1;">
          <div style="font-size: 1.4rem; font-weight: 800; color: ${statusColor};">Booking ${status}</div>
          <div style="font-size: 0.9rem; color: ${statusColor}99; margin-top: 4px;">Created on ${formatDate(createdOn)} at ${formatTime(createdOn)}</div>
        </div>
      </div>
      
      <!-- Booking ID & Status Grid -->
      <div style="display: grid; grid-template-columns: 1fr 1fr 1fr; gap: 16px;">
        <div style="background: rgba(99, 102, 241, 0.05); border: 1px solid rgba(99, 102, 241, 0.2); border-radius: 12px; padding: 16px;">
          <div style="font-size: 0.7rem; font-weight: 700; text-transform: uppercase; letter-spacing: 0.5px; color: #4338ca; margin-bottom: 8px;">Booking ID</div>
          <div style="font-size: 1rem; font-weight: 800; color: #1e1b4b; font-family: monospace; word-break: break-all;">${bookingId}</div>
        </div>
        <div style="background: rgba(34, 197, 94, 0.05); border: 1px solid rgba(34, 197, 94, 0.2); border-radius: 12px; padding: 16px;">
          <div style="font-size: 0.7rem; font-weight: 700; text-transform: uppercase; letter-spacing: 0.5px; color: #059669; margin-bottom: 8px;">Total Amount</div>
          <div style="font-size: 1.2rem; font-weight: 800; color: #065f46;">₹ ${amount.toFixed(2)}</div>
        </div>
        <div style="background: rgba(245, 158, 11, 0.05); border: 1px solid rgba(245, 158, 11, 0.2); border-radius: 12px; padding: 16px;">
          <div style="font-size: 0.7rem; font-weight: 700; text-transform: uppercase; letter-spacing: 0.5px; color: #b45309; margin-bottom: 8px;">Markup</div>
          <div style="font-size: 1rem; font-weight: 800; color: #92400e;">₹ ${markup.toFixed(2)}</div>
        </div>
      </div>
      
      <!-- Hotel Information -->
      <div style="background: linear-gradient(135deg, #f0f9ff 0%, #e0f2fe 100%); border: 1px solid rgba(59, 130, 246, 0.2); border-radius: 14px; padding: 20px;">
        <div style="display: flex; align-items: flex-start; gap: 16px; margin-bottom: 16px;">
          <div style="flex: 1;">
            <div style="font-size: 1.2rem; font-weight: 800; color: #1e3a8a; margin-bottom: 4px;">${hotelName}</div>
            <div style="display: flex; align-items: center; gap: 8px; margin-bottom: 8px;">
              ${Array(hotelRating).fill('<i class="ph ph-star-fill" style="color: #fbbf24; font-size: 0.9rem;"></i>').join('')}
              <span style="font-size: 0.85rem; color: #64748b;">${hotelRating} Star Hotel</span>
            </div>
            <div style="font-size: 0.85rem; color: #475569; line-height: 1.6;">
              <div><i class="ph ph-map-pin" style="margin-right: 6px;"></i>${address.adr || 'N/A'}</div>
              <div style="margin-top: 4px;"><i class="ph ph-building" style="margin-right: 6px;"></i>${address.ctn || 'N/A'}, ${address.sn || 'N/A'}, ${address.cn || 'N/A'}</div>
            </div>
          </div>
        </div>
        
        <!-- Check-in/Check-out Times -->
        <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 12px; margin-top: 16px; padding-top: 16px; border-top: 1px solid rgba(59, 130, 246, 0.2);">
          <div>
            <div style="font-size: 0.7rem; font-weight: 700; text-transform: uppercase; letter-spacing: 0.5px; color: #1e40af; margin-bottom: 6px;">Check-in</div>
            <div style="font-size: 0.95rem; font-weight: 700; color: #1e3a8a;">${checkInTime.beginTime || 'N/A'}</div>
            <div style="font-size: 0.8rem; color: #64748b; margin-top: 2px;">Min Age: ${checkInTime.minAge || 'N/A'}</div>
          </div>
          <div>
            <div style="font-size: 0.7rem; font-weight: 700; text-transform: uppercase; letter-spacing: 0.5px; color: #1e40af; margin-bottom: 6px;">Check-out</div>
            <div style="font-size: 0.95rem; font-weight: 700; color: #1e3a8a;">${checkOutTime.beginTime || 'N/A'}</div>
            <div style="font-size: 0.8rem; color: #64748b; margin-top: 2px;">Date: ${formatDate(checkOutDate)}</div>
          </div>
        </div>
      </div>
      
      <!-- Room Details -->
      <div style="background: linear-gradient(135deg, #faf5ff 0%, #f3e8ff 100%); border: 1px solid rgba(168, 85, 247, 0.2); border-radius: 14px; padding: 20px;">
        <div style="font-size: 0.9rem; font-weight: 700; text-transform: uppercase; letter-spacing: 0.5px; color: #7c3aed; margin-bottom: 12px;">Room Details</div>
        <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 16px;">
          <div>
            <div style="font-size: 0.75rem; font-weight: 700; text-transform: uppercase; letter-spacing: 0.5px; color: #7c3aed; margin-bottom: 6px;">Room Type</div>
            <div style="font-size: 1rem; font-weight: 700; color: #5b21b6;">${roomType}</div>
            <div style="font-size: 0.85rem; color: #a78bfa; margin-top: 4px;">${roomName}</div>
          </div>
          <div>
            <div style="font-size: 0.75rem; font-weight: 700; text-transform: uppercase; letter-spacing: 0.5px; color: #7c3aed; margin-bottom: 6px;">Meal Plan</div>
            <div style="font-size: 1rem; font-weight: 700; color: #5b21b6;">${mealPlan}</div>
          </div>
          <div>
            <div style="font-size: 0.75rem; font-weight: 700; text-transform: uppercase; letter-spacing: 0.5px; color: #7c3aed; margin-bottom: 6px;">Room Price</div>
            <div style="font-size: 1.1rem; font-weight: 800; color: #5b21b6;">₹ ${roomPrice.toFixed(2)}</div>
          </div>
          <div>
            <div style="font-size: 0.75rem; font-weight: 700; text-transform: uppercase; letter-spacing: 0.5px; color: #7c3aed; margin-bottom: 6px;">Guests</div>
            <div style="font-size: 1rem; font-weight: 700; color: #5b21b6;">${firstRoom.adt || 0} Adults, ${firstRoom.chd || 0} Children</div>
          </div>
        </div>
      </div>
      
      <!-- Guest Information -->
      <div style="background: linear-gradient(135deg, #fef3c7 0%, #fde68a 100%); border: 1px solid rgba(217, 119, 6, 0.2); border-radius: 14px; padding: 20px;">
        <div style="font-size: 0.9rem; font-weight: 700; text-transform: uppercase; letter-spacing: 0.5px; color: #b45309; margin-bottom: 12px;">Guest Information</div>
        <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 12px;">
          ${travellers.map((t, idx) => `
            <div style="background: white; border-radius: 10px; padding: 12px; border: 1px solid rgba(217, 119, 6, 0.1);">
              <div style="font-size: 0.75rem; font-weight: 700; text-transform: uppercase; letter-spacing: 0.5px; color: #b45309; margin-bottom: 6px;">Guest ${idx + 1}</div>
              <div style="font-size: 0.95rem; font-weight: 700; color: #92400e;">${t.fN} ${t.lN}</div>
              <div style="font-size: 0.8rem; color: #b45309; margin-top: 4px;">${t.ti} • ${t.pt}</div>
            </div>
          `).join('')}
        </div>
      </div>
      ...
      <!-- Contact Information -->
      <div style="background: linear-gradient(135deg, #dbeafe 0%, #bfdbfe 100%); border: 1px solid rgba(37, 99, 235, 0.2); border-radius: 14px; padding: 20px;">
        <div style="font-size: 0.9rem; font-weight: 700; text-transform: uppercase; letter-spacing: 0.5px; color: #1e40af; margin-bottom: 12px;">Contact Information</div>
        <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 16px;">
          <div>
            <div style="font-size: 0.75rem; font-weight: 700; text-transform: uppercase; letter-spacing: 0.5px; color: #1e40af; margin-bottom: 6px;">Email</div>
            <div style="font-size: 0.95rem; font-weight: 600; color: #1e3a8a; word-break: break-all;">
              ${deliveryInfo.emails?.join(', ') || 'N/A'}
            </div>
          </div>
          <div>
            <div style="font-size: 0.75rem; font-weight: 700; text-transform: uppercase; letter-spacing: 0.5px; color: #1e40af; margin-bottom: 6px;">Phone</div>
            <div style="font-size: 0.95rem; font-weight: 600; color: #1e3a8a;">
              ${deliveryInfo.code?.[0] || '+91'} ${deliveryInfo.contacts?.join(', ') || 'N/A'}
            </div>
          </div>
        </div>
      </div>
      ...
      <details style="background: #f8faff; border: 1px solid #e0e7ff; border-radius: 12px; padding: 16px;">
        <summary style="font-size: 0.85rem; font-weight: 700; color: #4338ca; cursor: pointer; user-select: none;">
          <i class="ph ph-caret-right" style="margin-right: 8px;"></i>View Full API Response
        </summary>
        <pre style="font-size: 0.7rem; color: #1e1b4b; overflow-x: auto; max-height: 400px; margin: 12px 0 0 0; padding-top: 12px; border-top: 1px solid #e0e7ff;">${JSON.stringify(data, null, 2)}</pre>
      </details>
      
      <!-- Action Buttons -->
      <div style="display: flex; gap: 12px; margin-top: 12px;">
        <button onclick="backToBookingDashboard()" style="flex: 1; padding: 14px 16px; background: #f3f4f6; color: #374151; border: 1px solid #d1d5db; border-radius: 10px; font-weight: 600; cursor: pointer; display: flex; align-items: center; justify-content: center; gap: 8px; transition: all 0.3s ease;" onmouseover="this.style.background='#e5e7eb';" onmouseout="this.style.background='#f3f4f6';">
          <i class="ph ph-arrow-left"></i> Back to Dashboard
        </button>
        <button onclick="window.print()" style="flex: 1; padding: 14px 16px; background: linear-gradient(135deg, #3b82f6 0%, #2563eb 100%); color: white; border: none; border-radius: 10px; font-weight: 600; cursor: pointer; display: flex; align-items: center; justify-content: center; gap: 8px; transition: all 0.3s ease; box-shadow: 0 4px 12px rgba(59, 130, 246, 0.3);" onmouseover="this.style.transform='translateY(-2px)'; this.style.boxShadow='0 6px 16px rgba(59, 130, 246, 0.4)';" onmouseout="this.style.transform='translateY(0)'; this.style.boxShadow='0 4px 12px rgba(59, 130, 246, 0.3)';">
          <i class="ph ph-printer"></i> Print Booking
        </button>
        <button onclick="cancelBookingFromDetail('${bookingId}')" style="flex: 1; padding: 14px 16px; background: linear-gradient(135deg, #ef4444 0%, #dc2626 100%); color: white; border: none; border-radius: 10px; font-weight: 600; cursor: pointer; display: flex; align-items: center; justify-content: center; gap: 8px; transition: all 0.3s ease; box-shadow: 0 4px 12px rgba(239, 68, 68, 0.3);" onmouseover="this.style.transform='translateY(-2px)'; this.style.boxShadow='0 6px 16px rgba(239, 68, 68, 0.4)';" onmouseout="this.style.transform='translateY(0)'; this.style.boxShadow='0 4px 12px rgba(239, 68, 68, 0.3)';">
...
// Function to go back to booking dashboard
function backToBookingDashboard() {
  window.location.href = '/ui/dashboard#bookings';
}

// Function to cancel booking from detail page
async function cancelBookingFromDetail(bookingId) {
  ...
  try {
    ...
    console.log('CANCEL_BOOKING_REQUEST', { duration: responseMs, status: res.status, ok: data.ok });
    
    if (!res.ok || data.ok === false) {
      container.innerHTML = `
        <div class="alert-box error fade-in" style="flex-direction: column; align-items: flex-start;">
          <div style="display: flex; gap: 8px; align-items: center; font-weight: 600;">
            <i class="ph ph-warning-circle" style="font-size: 1.2rem;"></i> Failed to Cancel Booking
          </div>
          <div style="margin-top: 8px; font-size: 0.9rem;">${data.reason || data.message || 'Please try again'}</div>
          <button onclick="location.reload()" style="margin-top: 12px; padding: 10px 16px; background: #3b82f6; color: white; border: none; border-radius: 8px; font-weight: 600; cursor: pointer;">
    ...
    container.innerHTML = `
      <div style="display: flex; flex-direction: column; gap: 20px;">
        <div style="background: linear-gradient(135deg, #10b98115 0%, #10b98108 100%); border: 2px solid #10b98140; border-radius: 16px; padding: 24px; display: flex; align-items: center; gap: 16px; box-shadow: 0 10px 30px #10b98120;">
          <div style="width: 60px; height: 60px; background: linear-gradient(135deg, #10b981 0%, #059669 100%); border-radius: 14px; display: flex; align-items: center; justify-content: center; color: white; font-size: 1.8rem; flex-shrink: 0;">
            <i class="ph ph-check-circle"></i>
          </div>
          <div style="flex: 1;">
            <div style="font-size: 1.4rem; font-weight: 800; color: #10b981;">Booking Cancelled Successfully</div>
            <div style="font-size: 0.9rem; color: #10b98199; margin-top: 4px;">Your booking has been cancelled. Refreshing details...</div>
          </div>
        </div>
      </div>`;
    
    // Wait 2 seconds then reload the booking detail
    setTimeout(() => {
      loadBookingDetailFromDashboard(bookingId);
    }, 2000);
    ...
    container.innerHTML = `
      <div class="alert-box error fade-in">
        <i class="ph ph-warning"></i>
        <span class="message">Error cancelling booking: ${err.message}</span>
      </div>`;
  }
}
...
function renderBookingDetail(data) {
  ...
  // Show review page (used for booking detail display)
  const reviewPage = document.getElementById("review-page");
  if (reviewPage) {
    reviewPage.classList.remove("hidden");
    reviewPage.classList.add("fade-in");
  ...
  const backBtn = reviewPage?.querySelector('.btn-ghost');
  if (backBtn) {
    backBtn.innerHTML = '<i class="ph ph-arrow-left"></i> Back to Search';
  ...
  const bookingDetailsHtml = `
    <div style="display: flex; flex-direction: column; gap: 24px;">
      ...
      <!-- Action Buttons -->
      <div style="display: flex; gap: 12px; margin-top: 12px;">
        <button onclick="backToSearch()" style="flex: 1; padding: 14px 16px; background: #f3f4f6; color: #374151; border: 1px solid #d1d5db; border-radius: 10px; font-weight: 600; cursor: pointer; display: flex; align-items: center; justify-content: center; gap: 8px; transition: all 0.3s ease;" onmouseover="this.style.background='#e5e7eb';" onmouseout="this.style.background='#f3f4f6';">
...
async function bookRoom(optionId, correlationId) {
  ...
  try {
    ...
    const responseMs = Math.round(performance.now() - bookStartTime);
    const data = await res.json();
    
    console.log('API_BOOK_REQUEST', { duration: responseMs, status: res.status, ok: data.ok });
    
    // Track booking response time
    journeyResponseTimes.book = responseMs;
    displayResponseTimes();
